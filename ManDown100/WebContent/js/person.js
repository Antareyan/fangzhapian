...
Person.prototype = {
		...
		initSprite : function(){//初始化精灵
			...
			sprite.add("down",new WF.sprite.Animation({startX:64,sw:64,sh:64,width:53,height:53}));
			sprite.add("normal",new WF.sprite.Animation({sw:64,sh:64,width:53,height:53}));
			sprite.add("up",new WF.sprite.Animation({startX:128,sw:64,sh:64,width:53,height:53}));
			sprite.add("right",new WF.sprite.Animation({startX:320,fs:2,sw:64,sh:64,width:53,height:53,loop:true}));
			sprite.add("left",new WF.sprite.Animation({startX:192,fs:2,sw:64,sh:64,width:53,height:53,loop:true}));
...
//			alert("2");
		...
		goUp : function(){//正在向上
			...
			//y轴加速度, y轴加速度增量
			this.sprite.setYSpeed(this.yspeed*-3,0.4);
		},
